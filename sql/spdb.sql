    ...
    BEGIN
        RETURN QUERY
            SELECT * FROM pgr_astar('SELECT gid AS id,
                         source::integer,
                         target::integer,
                         sign(cost)*length*('||w||'+((1-'||w||')/(least(maxspeed_forward,'||v||'))))::double precision AS cost,
                         sign(reverse_cost)*length*('||w||'+((1-'||w||')/(least(maxspeed_backward,'||v||'))))::double precision AS reverse_cost,
                         x1, y1, x2, y2
